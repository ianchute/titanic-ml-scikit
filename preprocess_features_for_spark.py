...
preprocessed_test_ids = test_df.get('PassengerId').values.tolist()
...
def encode():
    ...
    all_features_encoded = [preprocessed_labels + preprocessed_test_ids] + all_features_encoded
...
np.savetxt('data/train_clean.csv', all_features_encoded_train, fmt='%s', delimiter=',')
np.savetxt('data/test_clean.csv', all_features_encoded_test, fmt='%s', delimiter=',')
